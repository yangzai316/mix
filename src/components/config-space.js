		...
		>
					...
					<Divider orientation="left">样式设置</Divider>
					{
						styles.map((item, index) => {
							if (item.type === 'input') {
								...
								>
									<Input value={item.value} onChange={(e) => { change(e, item.key, 'style') }} />
								</Form.Item>
									...
									<Radio.Group value={item.value}>
										{
											styleAttrs[item.key].map(o => {
												return <Tooltip key={o.key} placement="top" title={o.title}>
				...
				attribute && <>
					<Divider orientation="left">属性设置</Divider>
					{
						Object.keys(attribute).map((key, i) => {
							...
							>
								<Input value={`${attribute[key]}`} onChange={(e) => { change(e, key, 'attribute') }} />
							</Form.Item>
						})
					}
				</>
			}
			{
				(content || content === '') && <>
					...
					<Input value={`${content}`} onChange={(e) => { change(e, '', 'content') }} />
				</>
			}
		</Form>
	</>
